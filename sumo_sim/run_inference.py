...
GUI = True
SEED = 42
ESP32_IP = "10.103.0.208"   # change this

# ===============================
# Send SUMO state string to ESP32
# ===============================
def send_to_esp32(state_str):
    """
    Sends the SUMO red/yellow/green state string to ESP32
    Example state_str: 'rGr' or 'ryG'
    """
    try:
        url = f"http://{ESP32_IP}/set?state={state_str}"
        r = requests.get(url, timeout=1)

        if r.status_code == 200:
            print(f"[ESP32] ✅ State {state_str} sent successfully")
        else:
            print(f"[ESP32] ⚠️ HTTP {r.status_code}")
...
def run_simulation():
    ...
    try:
        while not done:
            ...
            # Only send to ESP32 when state changes
            if current_state != last_state:
                send_to_esp32(current_state)
